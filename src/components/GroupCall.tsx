...
export const GroupCall = ({ groupId, userId, roomId, callId, type, onClose, inline }: { groupId?: string, userId?: string, roomId?: string, callId?: string, type: 'voice' | 'video' | 'walkie-talkie', onClose: () => void, inline?: boolean }) => {
  ...
  // Silent microphone telemetry logger during call (does NOT invoke getUserMedia to avoid interrupting active media stream)
  useEffect(() => {
    if (connectionStage !== 'established' || isMuted) return;

    if (localMicDb <= -58) {
      const timer = setTimeout(() => {
        console.log('[Diagnostic][AutoMic] Ambient silence detected (user quiet or paused). Active mic stream preserved.');
      }, 4000);
      return () => clearTimeout(timer);
    }
  }, [localMicDb, connectionStage, isMuted]);
    ...
    const handleRemoteStream = (e: any) => {
      ...
      setParticipants(prev => {
        if (!prev.find(p => p.id === from)) {
           ...
           return [...prev, { id: u.id, name: u.displayName || u.id, avatar: u.avatar, isMuted: false, isVideoOff: false, isSpeaking: false, status: 'online' }];
        }
        return prev;
      });
